	    ...
	    <div class="pt56" v-if="success">
			...
			<div v-if="activeTab === 'tab1'">
	            <mu-list v-if="user.recent_replies.length>0">
					<router-link v-for="item in user.recent_replies" :to="{name:'topic',params:{id:item.id}}">
			...
			<div v-if="activeTab === 'tab2'">
				<mu-list v-if="user.recent_topics.length>0">
					<router-link v-for="item in user.recent_topics" :to="{name:'topic',params:{id:item.id}}">
					    <mu-list-item :title="item.title" :describeText="item.author.loginname">
					      	<mu-avatar :src="item.author.avatar_url" slot="leftAvatar"/>
					      	<div class="time">{{item.last_reply_at|getTimeStr}}</div>
					    </mu-list-item>
...
<style lang="scss">
	.user{
		...
		.gridlist-demo{
			width: 100%;
			background:url(http://img105.job1001.com/upload/album/2014-08-22/1408669832_5g221QL9.jpg) 0 40%;
